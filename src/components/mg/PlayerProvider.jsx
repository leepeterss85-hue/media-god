...
export function PlayerProvider({ children }) {
  ...
  const play = useCallback(
    async (request = {}) => {
      const originalSources = Array.isArray(
        request?.sources
      )
        ? request.sources
            .map(normaliseOriginalSource)
            .filter(Boolean)
        : [];
      ...
      const addonPromise =
        !isLive && !request?.skipAddonLookup
          ? fetchAddonSources({
              imdbId,
              tmdbId,
              title: request?.title || "",
      ...
      const [addonLookup, rdLibrarySource] =
        await Promise.all([
          addonPromise,
          rdLibraryPromise,
        ]);
      ...
      const nonPlaybackSources = orderedSources.filter(
        (item) => !playbackSources.includes(item)
      );
        ...
        orderedSources = [
          {
            label:
              addonLookup?.reason ||
              (imdbId
                ? "No playable source was returned by your configured addons."
                : "IMDb id could not be resolved for this title."),
            type: "status",
            src: "",
            url: "",
      ...
      setSource({
        ...
        rdYear:
          ...
          null,
        rdSeason:
          request?.rdSeason ?? season,
        rdEpisode:
          request?.rdEpisode ?? episode,
        ...
        sourceDiagnostics: {
          ...
          tmdbId,
          addonsChecked: Number(
            addonLookup?.addonsChecked || 0
          ),
          diagnostics:
            addonLookup?.diagnostics || [],
          reason: addonLookup?.reason || "",
          discoveredCount: Array.isArray(
            addonLookup?.streams
          )
