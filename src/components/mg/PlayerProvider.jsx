...
const scoreSource = (item) => {
  ...
  const directBonus = isPlayableDirectSource(item) ? 5000 : 0;
...
const resolveImdbId = async ({
  ...
  if (imdbId || imdb_id) return imdbId || imdb_id;
...
const fetchAddonStreams = async ({
  ...
  /*
   * Series stream requests normally
   * require a concrete episode.
   */
  if (
    isSeries &&
    (
...
export function PlayerProvider({
  ...
  const play = useCallback(
    async (
      ...
      /*
       * Users without RD should
       * never be automatically sent
       * to a magnet.
       */
      const usableSources =
        hasRd
...
export function PlayerProvider({ children }) {
  ...
  const play = useCallback(
    async (s) => {
      ...
      const directSources = sources.filter((item) => {
        const url = getSourceUrl(item);
        return (
          /^https?:\/\//i.test(url) &&
          !isMagnetSource(item)
      ...
      const magnetSources = sources.filter(isMagnetSource);
      const remainingSources = sources.filter(
        (item) =>
          !directSources.includes(item) &&
          !magnetSources.includes(item)
      );
      ...
      const playableSource =
        orderedSources[0] || sources[0];

      const activeUrl =
        getSourceUrl(playableSource) ||
        s?.src ||
        "";
