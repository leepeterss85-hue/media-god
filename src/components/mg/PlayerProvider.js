...
function PlayerAutomationBridge({ children }) {
  ...
  useEffect(() => {
    ...
    const onPlaySpecificEpisode = async (event) => {
      ...
      const seasonNumber = positiveInt(
        event?.detail?.seasonNumber ??
          event?.detail?.season
      );

      const suppliedEpisode =
        event?.detail?.episodeItem ||
        (event?.detail?.episode != null
          ? {
              episode_number:
                event.detail.episode,
            }
          : null);

      if (!seasonNumber || !suppliedEpisode) {
        return;
      }
      ...
      try {
        publishStatus(
          `Loading S${String(seasonNumber).padStart(
            2,
            "0"
          )}E${String(
            positiveInt(
              suppliedEpisode?.episode_number
            ) || 1
          ).padStart(2, "0")}…`
        );
