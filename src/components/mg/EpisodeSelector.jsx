...
export default function EpisodeSelector({ item, seasons, trailerUrl, providers }) {
  ...
  useEffect(() => {
    ...
    setEpisodes([]);
    base44.functions
      .invoke("getTmdbMovies", { media_type: "tv", movie_id: item.id, season_number: season })
  ...
  const seasonLabel = currentSeason ? currentSeason.name : "Select season";

  const playEpisode = (ep) => {
    const title = `${item.title} — S${season}E${ep.episode_number}`;
    player.play({
      title,
  ...
  return (
    <div className="mt-5">
      <h3 className="text-white/80 text-xs font-bold uppercase tracking-wider mb-2">Episodes</h3>
      ...
      <div className="relative mb-3">
        ...
        >
          ...
          <ChevronDown className={"w-4 h-4 text-white/60 transition-transform " + (pickerOpen ? "rotate-180" : "")} />
        </button>
        {pickerOpen && (
            ...
            {seasons.map((s) => (
              <button
                ...
                className={
                  ...
                  (String(s.season_number) === season ? "text-mg-green" : "text-white/80")
                }
              >
                <span>{s.name}</span>
                <span className="text-white/40 text-xs">{s.episode_count} ep</span>
              </button>
            ))}
          </div>
        )}
        ...
        <div className="space-y-2">
          {episodes.map((ep) => (
            ...
            >
              ...
              >
                {ep.still_url ? (
                  <Image src={ep.still_url} alt={ep.name} className="w-full h-full object-cover" fittingType="fill" />
                ) : (
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-white/50 text-xs mt-1 line-clamp-2">{ep.overview || "No description."}</p>
              </div>
            </div>
          ))}
          {episodes.length === 0 && (
            <p className="text-white/40 text-xs">No episodes found for this season.</p>
          )}
        </div>
      )}
    </div>
