...
const TMDB_KEY = "38267272847a9ef3878b273b37963d76";

const getBaseUrl = (manifestUrl) =>
  String(manifestUrl || "")
...
export default function StreamSourcesBox({
   ...
   * FIX:
   *
   * The previous code assumed findChannelsByTitle()
   * always returned a Promise and immediately called .then().
   *
   * In the running Base44 build it can also return an
   * already-resolved value, which caused:
   *
   * findChannelsByTitle(...).then is not a function
   *
   * Promise.resolve() safely accepts BOTH:
   * - an array/value
   * - a Promise
   */
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    /*
     * Movie detail can be looked up immediately.
     *
     * TV streams are resolved after an episode
     * is selected because season/episode are then known.
     */
    if (
      mediaType === "tv"
    ) {
