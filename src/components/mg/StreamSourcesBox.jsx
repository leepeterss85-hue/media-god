...
const normaliseAddonStream = (stream, addon) => {
  ...
  const magnet = String(url).toLowerCase().startsWith("magnet:") || !!infoHash;
  const rawTitle = stream?.title || stream?.name || stream?.filename || "Stream Source";

  return {
    id: `${addon?.name || "addon"}-${Math.random().toString(36).slice(2)}`,
    kind: "addon-stream",
    label: String(rawTitle).split("\n")[0],
    note: stream?.name || addon?.name || "Addon source",
    url,
    src: url,
    infoHash: infoHash || undefined,
    type: magnet ? "rd" : "url",
    behaviorHints: stream?.behaviorHints || stream?.behavior_hints || {},
...
export default function StreamSourcesBox({
      ...
      try {
        ...
        if (!resolvedImdb && tmdbId != null) {
          try {
            ...
            // PlayerProvider will also attempt the same resolution when playback starts.
          }
        }

        if (!resolvedImdb) {
          if (!cancelled) setScrapedStreams([]);
          return;
        }

        const media = isSeries ? "series" : "movie";
        const endpointId = resolvedImdb;
        ...
        const results = await Promise.all(
          activeAddons.map(async (addon) => {
            try {
              ...
              const targetUrl = `${baseUrl}/stream/${media}/${endpointId}.json`;
              ...
              try {
                ...
                return data.streams
                  .map((stream) => normaliseAddonStream(stream, addon))
                  .filter(Boolean);
              } finally {
        ...
        if (!cancelled) {
          const seen = new Set();
          const unique = results.flat().filter((stream) => {
            const key = String(stream?.url || "").trim();
            if (!key || seen.has(key)) return false;
            seen.add(key);
            return true;
          });
          setScrapedStreams(unique.slice(0, 25));
        }
      } catch {
        if (!cancelled) setScrapedStreams([]);
      } finally {
  ...
  const sources = [
    {
      ...
      kind: "rd",
      label: "Real-Debrid",
      note: "Find a cached stream or resolve a torrent",
    },
    ...scrapedStreams,
    {
      id: "paste",
    ...
    ...(trailerUrl
      ? [{ id: "trailer", kind: "trailer", label: "Trailer", note: "YouTube" }]
      : []),
    ...(liveMatches || []).map((c, i) => ({
      id: `live-${i}`,
      kind: "live",
    ...
    })),
    {
      ...
      kind: "archive",
      label: "Free Archive",
      note: "Public-domain Internet Archive",
    },
        ...
        sources: [
          {
            ...
            src: s.src,
            url: s.url,
            magnet: s.type === "rd" ? s.url : undefined,
            infoHash: s.infoHash,
  ...
  const rowClass = (kind) =>
    cn(
      ...
      kind === "rd" && "bg-mg-green/10 hover:bg-mg-green/20 border-mg-green/30",
      kind === "addon-stream" && "bg-cyan-500/10 hover:bg-cyan-500/20 border-cyan-500/30",
      kind === "live" && "bg-red-500/10 hover:bg-red-500/20 border-red-500/30",
      !["rd", "addon-stream", "live"].includes(kind) &&
    ...
    <div className="mt-4 bg-mg-card border border-white/10 rounded-lg p-3">
      ...
      {loading ? (
        ...
        <div className="flex flex-col gap-1.5">
          ...
          {!scraping && scrapedStreams.length === 0 && (
            <p className="text-[10px] text-white/30 px-1 pt-1">
