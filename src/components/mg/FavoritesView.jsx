  ...
  if (loading) {
    return (
      <div data-mg-library-view="true" data-mg-favorites-view="true" className="p-4 max-w-6xl mx-auto">
        ...
        <div data-mg-library-grid="true" className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 gap-3">
  ...
  return (
    <div className="p-4 max-w-6xl mx-auto">
      ...
      {items.length === 0 ? (
        ...
        <div data-mg-library-grid="true" className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 gap-3">
          {items.map((it) => (
            <div key={it.id} data-mg-favorite-card="true" className="mg-fire-tv-library-card group">
              <div className="relative aspect-[2/3] rounded-md overflow-hidden border border-white/10 bg-mg-card">
                ...
                <button onClick={() => play(it)} className="mg-hover-action absolute inset-0 flex items-center justify-center bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity" aria-label={`Play ${it.title}`}>
                ...
                <button onClick={() => remove(it.id, it.title)} className="mg-hover-action absolute top-1.5 right-1.5 w-7 h-7 rounded-full bg-black/60 text-white/80 hover:text-white hover:bg-red-600 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity" aria-label={`Remove ${it.title} from Favorites`}>
