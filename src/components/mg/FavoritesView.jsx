...
export default function FavoritesView() {
  ...
  const removeSelected = async () => {
    ...
    const confirmed =
      ...
      window.confirm(
        `Remove ${selectedIds.length} selected favorite${selectedIds.length === 1 ? "" : "s"}?`
      );
    ...
    for (const item of items.filter((row) => wanted.has(row.id))) {
      try {
        await base44.entities.Favorite.delete(item.id);
      } catch {
        failed += 1;
      }
    }
    ...
    toast({
      title: failed ? "Favorites partly updated" : "Selected favorites removed",
      description: failed
        ? `${failed} favorite${failed === 1 ? "" : "s"} could not be removed.`
        : undefined,
  ...
  const clearAll = async () => {
    ...
    const confirmed =
      typeof window === "undefined" ||
      window.confirm(
        `Remove all ${items.length} favorite${items.length === 1 ? "" : "s"}?`
      );
    ...
    if (failed === 0) {
      ...
      await load();
      toast({
        title: "Favorites partly cleared",
        description: `${failed} favorite${failed === 1 ? " could" : "s could"} not be removed.`,
        variant: "destructive",
      });
    }
