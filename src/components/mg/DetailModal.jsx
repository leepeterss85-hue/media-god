...
export default function DetailModal({
  ...
  /*
   * Load the full TMDB details.
   *
   * For TV this includes the seasons array.
   */
  useEffect(() => {
    let cancelled =
      false;
  ...
  /*
   * Movies still use the normal Play button.
   *
   * TV shows no longer go straight into the player
   * without a season / episode.
   */
  const playMovie =
    async () => {
      await player.play({
  ...
  /*
   * For TV, the main button now scrolls the user
   * to the episode selector instead of opening an
   * incomplete source search.
   */
  const goToEpisodes =
    () => {
      const target =
        ...
        <div className="px-4 md:px-6 -mt-20 md:-mt-24 relative">
          ...
          <div className="flex gap-2 mt-4">
            <button
              ...
              className="flex-1 flex items-center justify-center gap-2 bg-mg-green text-black font-semibold text-sm py-2.5 rounded-lg hover:bg-mg-green-dim disabled:opacity-60"
            >
              {resolvedMediaType ===
              "tv" ? (
                loading ? (
