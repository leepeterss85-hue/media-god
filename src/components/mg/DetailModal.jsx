...
export default function DetailModal({ item, mediaType, onClose }) {
  ...
  useEffect(() => {
    ...
    setData(null);
    base44.functions
      .invoke("getTmdbMovies", { media_type: mediaType, movie_id: item.id })
      ...
      >
        ...
        <div className="relative h-44 md:h-56 bg-mg-card">
          {details.backdrop_url ? (
            <Image src={details.backdrop_url} alt={item.title} className="w-full h-full object-cover" fittingType="fill" />
          ) : item.poster_url ? (
        ...
        <div className="px-4 md:px-6 -mt-20 md:-mt-24 relative">
          <div className="flex gap-4 items-end">
            ...
            <div className="flex-1 min-w-0 pb-2">
              <h2 className="text-white font-bold text-lg md:text-2xl leading-tight">{item.title}</h2>
              <div className="flex flex-wrap items-center gap-2 mt-1.5 text-xs text-white/60">
                ...
                {details.rating && (
                  <span className="flex items-center gap-1 text-mg-green">
                    <Star className="w-3 h-3 fill-mg-green" /> {details.rating}
                  </span>
                )}
                {details.runtime ? (
                  <span className="flex items-center gap-1">
                    <Clock className="w-3 h-3" /> {details.runtime}m
                  </span>
                ) : null}
              </div>
          ...
          {/* Plot */}
          <div className="mt-5">
            <h3 className="text-white/80 text-xs font-bold uppercase tracking-wider mb-1.5">Plot</h3>
          ...
          <div className="mt-5">
            <h3 className="text-white/80 text-xs font-bold uppercase tracking-wider mb-2 flex items-center gap-1.5">
              <Tv className="w-3.5 h-3.5 text-mg-green" /> Where to Watch
            </h3>
            {loading ? (
              <div className="flex gap-2">
                {Array.from({ length: 4 }).map((_, i) => (
                  <div key={i} className="w-12 h-12 rounded-md bg-mg-card animate-pulse" />
                ))}
              </div>
            ) : providers.length > 0 ? (
          ...
          <div className="mt-5 pb-6">
            <h3 className="text-white/80 text-xs font-bold uppercase tracking-wider mb-2">Cast</h3>
            {loading ? (
              ...
              <div className="flex gap-3 overflow-x-auto pb-1">
                {cast.map((c) => (
                  <div key={c.name + c.character} className="shrink-0 w-16 text-center">
                    <div className="w-16 h-16 rounded-full overflow-hidden border border-white/10 bg-mg-card">
                      {c.profile_url ? (
                        <Image src={c.profile_url} alt={c.name} className="w-full h-full object-cover" fittingType="fill" />
                      ) : (
                    ...
                    <p className="text-white text-[11px] font-medium mt-1.5 truncate">{c.name}</p>
                    <p className="text-white/40 text-[10px] truncate">{c.character}</p>
                  </div>
                ))}
              </div>
            ) : (
              <p className="text-white/40 text-xs">No cast information available.</p>
            )}
          </div>
        </div>
      </div>
