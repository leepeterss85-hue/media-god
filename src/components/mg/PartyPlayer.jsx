...
const buildDiagnosticLabel = ({
  ...
  parts.push(
    `Addons ${Number(addonLookup?.addonsChecked || 0)}`
  );
    ...
    async (request = {}) => {
      ...
      console.info(
        "[Media God] Playback diagnostics",
        {
          ...
          season,
          episode,
          imdbStatus: imdbInfo?.status,
          addonLookupStatus: addonLookup?.status,
          addonsChecked: addonLookup?.addonsChecked,
          ...
          rdConnected: hasRd,
          rdLookupStatus: rdLookup?.status,
