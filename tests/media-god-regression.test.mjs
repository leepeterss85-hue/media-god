...
test("catalogue screens only publish movies and TV episodes that have real addon sources", () => {
  const availabilitySource = readFileSync(
    new URL("../src/components/mg/sourceAvailability.js", import.meta.url),
    "utf8"
  );
  const homeSource = readFileSync(
  ...
  const movieSource = readFileSync(
    new URL("../src/components/mg/MoviesView.jsx", import.meta.url),
    "utf8"
  );
  ...
  const newEpisodesSource = readFileSync(
    new URL("../src/components/mg/NewEpisodesRow.jsx", import.meta.url),
    "utf8"
  );
  ...
  assert.match(
    availabilitySource,
    /invoke\("fetchAddonStreams"[\s\S]{0,500}?fast_mode:\s*true/
  );
  assert.match(
    availabilitySource,
    /streams\.some\(streamLooksUsable\)/
  );
  assert.match(
    availabilitySource,
  ...
  assert.match(
    mediaRowSource,
    /sourceAvailableOnly[\s\S]{0,1400}?filterItemsWithPlayableSources/
  );
  assert.match(
    heroSource,
    /sourceAvailableOnly[\s\S]{0,1400}?filterItemsWithPlayableSources/
  ...
  assert.match(
    homeSource,
    /<HeroSlider[\s\S]{0,220}?sourceAvailableOnly/
  );
  assert.match(
    homeSource,
    /<StreamingServiceRows[\s\S]{0,500}?sourceAvailableOnly/
  );
  assert.match(
    movieSource,
    /filterItemsWithPlayableSources\([\s\S]{0,220}?candidates/
  );
  assert.match(
    episodeSource,
